...
urlpatterns = [
    ...
    path('find_posts/<int:post_pk>/comment/', FindCommentViewSet.as_view({'get': 'list', 'post': 'create'}), name='comment-list-for-post'),
    path('find_posts/<int:post_pk>/comment/<int:pk>/', FindCommentViewSet.as_view({'get': 'retrieve', 'delete': 'destroy'}), name='comment-detail-for-post'),
    path('find_posts/<int:pk>/reply/', FindReplyViewSet.as_view({'get': 'list', 'post': 'create'}), name='reply-list'),
    path('find_posts/<int:post_pk>/reply/<int:pk>/', FindReplyViewSet.as_view({'get': 'retrieve', 'delete': 'destroy'}),
         name='reply-detail'),
]
